...
INTRO_MESSAGE=(
    "Anthias адаптовано для Orange Pi Zero 3."
    "Після встановлення регулярний робочий стіл може бути недоступним."
    ""
    "Можна вибрати версію:"
    "  - **latest:** Остання версія з гілки \`master\`."
    "  - **tag:** Конкретна версія за тегом."
    ""
    "Увага: \`latest\` - це rolling release."
)
MANAGE_NETWORK_PROMPT=(
    "Дозволити Anthias керувати мережею?"
)
VERSION_PROMPT=(
    "Яку версію Anthias встановити?"
)
...
SYSTEM_UPGRADE_PROMPT=(
    "Виконати повне оновлення системи?"
)
SUDO_ARGS=()
...
# Функція для встановлення gum та jq
...
function install_packages() {
    ...
    # Для Orange Pi не потрібно заміняти джерела, як для Raspberry Pi
    sudo apt update -y
